...
GENESIS_COINS="5000000000000uxprt,5000000000000uatom,5000000000000ukava,5000000000000stake"
...
echo $MNEMONIC_4 | $BINARY keys add user3 --home $CHAIN_DIR/$CHAIN_ID --recover --keyring-backend=test 
$BINARY add-genesis-account $($BINARY --home $CHAIN_DIR/$CHAIN_ID keys show validator --keyring-backend test -a) --home $CHAIN_DIR/$CHAIN_ID $GENESIS_COINS
$BINARY add-genesis-account $($BINARY --home $CHAIN_DIR/$CHAIN_ID keys show user1 --keyring-backend test -a) --home $CHAIN_DIR/$CHAIN_ID $GENESIS_COINS
$BINARY add-genesis-account $($BINARY --home $CHAIN_DIR/$CHAIN_ID keys show user2 --keyring-backend test -a) --home $CHAIN_DIR/$CHAIN_ID $GENESIS_COINS
$BINARY add-genesis-account $($BINARY --home $CHAIN_DIR/$CHAIN_ID keys show user3 --keyring-backend test -a) --home $CHAIN_DIR/$CHAIN_ID $GENESIS_COINS
